...
stty -F $tty 1200 cs8 -echo -brkint -imaxbel -ixon # set up the serial port. 1200 baud, 8 bit character size, no echo, disable breaks causing an interrupt signal, flush full input buffer on a character, disable xon/xoff flow control.
